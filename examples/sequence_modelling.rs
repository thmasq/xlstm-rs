...
fn generate_sine_wave_sequence(length: usize, frequency: f64, phase: f64, noise_level: f64) -> Vec<Array2<f64>> {
    ...
    for i in 0..length {
        ...
        let noise = if noise_level > 0.0 {
            use rand::Rng;
            (rng.gen::<f64>() - 0.5) * 2.0 * noise_level
        } else {
            0.0
        };
...
fn main() {
    ...
    println!("   Simple sine wave sequence: {} time steps", train_sine.len());
    println!("   Complex multi-frequency sequence: {} time steps", train_complex.len());
    ...
    println!("   mLSTM network: {} parameters", mlstm_net.num_parameters());
    println!("   sLSTM network: {} parameters", slstm_net.num_parameters());
    println!("   Mixed network: {} parameters", mixed_net.num_parameters());
    ...
    println!("   Processed {} time steps with each network", sine_outputs_mlstm.len());
    ...
    println!("     mLSTM - mean: {:.4}, std: {:.4}", 
             last_mlstm.mean().unwrap(), 
             last_mlstm.std(0.0));
    println!("     sLSTM - mean: {:.4}, std: {:.4}", 
             last_slstm.mean().unwrap(),
             last_slstm.std(0.0));
    println!("     Mixed - mean: {:.4}, std: {:.4}", 
    ...
    for step in 0..predict_steps {
        ...
        println!("   Step {}: predicted value = {:.4}", step + 1, prediction[[0, 0]]);
    }
    ...
    for step in 0..predict_steps {
        ...
        if actual_idx < pred_sequence.len() {
            ...
            println!("     Step {}: actual={:.4}, predicted={:.4}, error={:.4}", 
                     step + 1, actual, predicted, error);
        }
    }
    ...
    for segment in 0..5 {
        ...
        if start_idx < end_idx {
            ...
            let mean_response = segment_outputs.iter()
                .map(|output| output.mean().unwrap())
                .sum::<f64>() / segment_outputs.len() as f64;
                
            println!("   Segment {} (t={}-{}): mean response = {:.4}", 
                     segment + 1, start_idx, end_idx - 1, mean_response);
        }
    }
    ...
    for i in 0..memory_test_len {
        let base_val = if i < 10 { 1.0 } else if i > 70 { 1.0 } else { 0.0 };
        let noise = (i as f64 * 0.3).sin() * 0.2;
        
        let input = arr2(&[[base_val + noise], [noise], [base_val], [(i as f64) / memory_test_len as f64]]);
    ...
    let early_mean = memory_outputs[..10].iter()
        .map(|out| out.mean().unwrap())
        .sum::<f64>() / 10.0;
    let late_mean = memory_outputs[70..].iter()
        .map(|out| out.mean().unwrap())  
        .sum::<f64>() / (memory_outputs.len() - 70) as f64;
    
    println!("   Early sequence mean response: {:.4}", early_mean);
    println!("   Late sequence mean response: {:.4}", late_mean);
    println!("   Response correlation: {:.4}", 
             if early_mean.abs() > 1e-6 { late_mean / early_mean } else { 0.0 });
    ...
    let mlstm_final = mlstm_net.forward_sequence(&test_seq).last().unwrap().clone();
    let slstm_final = slstm_net.forward_sequence(&test_seq).last().unwrap().clone();
    let mixed_final = mixed_net.forward_sequence(&test_seq).last().unwrap().clone();
    let deep_final = deep_net.forward_sequence(&test_seq).last().unwrap().clone();
    
    println!("   Final outputs on test sequence:");
    println!("   mLSTM: [{:.4}, {:.4}, {:.4}, {:.4}]", 
             mlstm_final[[0, 0]], mlstm_final[[1, 0]], mlstm_final[[2, 0]], mlstm_final[[3, 0]]);
    println!("   sLSTM: [{:.4}, {:.4}, {:.4}, {:.4}]", 
             slstm_final[[0, 0]], slstm_final[[1, 0]], slstm_final[[2, 0]], slstm_final[[3, 0]]);
    println!("   Mixed: [{:.4}, {:.4}, {:.4}, {:.4}]", 
             mixed_final[[0, 0]], mixed_final[[1, 0]], mixed_final[[2, 0]], mixed_final[[3, 0]]);
    println!("   Deep:  [{:.4}, {:.4}, {:.4}, {:.4}]", 
             deep_final[[0, 0]], deep_final[[1, 0]], deep_final[[2, 0]], deep_final[[3, 0]]);
